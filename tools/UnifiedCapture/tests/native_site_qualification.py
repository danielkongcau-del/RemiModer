...
def main():
    ...
    try:
        ...
        for name in ("pair_entry", "pair_exit"):
            target = targets[name]
            near_only = name == "pair_entry"
            qualification["sites"].append({"id": name, "module": "fixture", "rva": target["rva"],
                "verified_source_prefix": target["expected_prefix"],
                "semantic_safe_span": 6 if near_only else 16,
                "safe_redirect_spans": [5] if near_only else [5, 16],
        ...
        if any(row["backend_patch_contract"]["required_redirect_span"] not in (5, 16)
            ...
            raise AssertionError(response)
        if response["sites"][0]["backend_patch_contract"]["required_redirect_span"] != 5:
            raise AssertionError("near-only qualification accepted a far redirect")
        ...
        plan = {"schema": "uc.capture-plan.v2", "plan_id": "qualified-entry-fixture", "plan_revision": 1,
            ...
            "observations": [{"id": "pair-entry", "backend": "gum_function_probe_pair", "module": "fixture",
                "instruction_site_id": "fixture-pair-entry-instruction",
